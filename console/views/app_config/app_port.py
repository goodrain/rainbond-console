...
class AppPortView(AppBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        port_list = []
        # 判断是否开启hostnetwork
        hostnetwork = k8s_attribute_repo.get_by_component_id_name(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not port:
            return Response(general_message(400, "params error", "缺少端口参数"),
                            status=400)
        if not protocol:
            return Response(general_message(400, "params error", "缺少协议参数"),
                            status=400)
        if not port_alias:
...
class AppPortManageView(AppBaseView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        variables = port_service.get_port_variables(self.tenant, self.service,
                                                    port_info)
