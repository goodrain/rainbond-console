...
class ServiceShareRecordView(RegionTenantHeaderView):
    def get(self, request, team_name, group_id, *args, **kwargs):
        ...
        total, share_records = share_repo.get_service_share_records_by_groupid(team_name=team_name,
                                                                               group_id=group_id,
                                                                               page=page,
                                                                               page_size=page_size)
        ...
        for share_record in share_records:
            ...
            if scope == "goodrain" and store_id and share_record.app_id and not app_model_name:
                try:
                    ...
                    if not mkt:
                        mkt = app_market_service.get_app_market_by_name(self.tenant.enterprise_id,
...
class ServiceShareRecordInfoView(RegionTenantHeaderView):
    def get(self, request, team_name, group_id, record_id, *args, **kwargs):
        ...
        if share_record:
            ...
            scope = share_record.scope
            if store_id:
                extend, market = app_market_service.get_app_market(self.tenant.enterprise_id,
                                                                   share_record.share_app_market_name,
                                                                   extend="true",
                                                                   raise_exception=True)
                if market:
...
class ServiceShareInfoView(RegionTenantHeaderView):
    ...
    def post(self, request, team_name, share_id, *args, **kwargs):
        ...
        code, msg, bean = share_service.create_share_info(tenant=self.tenant,
...
class AppMarketAppModelLView(JWTAuthApiView):
    def get(self, request, enterprise_id, market_name, *args, **kwargs):
        ...
        if is_plugin == "true":
            data, page, page_size, total = app_market_service.get_market_plugins_apps(market_model,
                                                                                      page,
                                                                                      page_size,
                                                                                      query=query,
                                                                                      query_all=query_all,
            ...
            data, page, page_size, total = app_market_service.get_market_app_list(market_model,
                                                                                  page,
                                                                                  page_size,
                                                                                  query=query,
                                                                                  query_all=query_all,
