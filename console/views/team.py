    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if tenants:
                ...
                data = {"active_teams": active_tenants, "new_join_team": teams_list[0], "teams": teams_list}
...
class TeamRegionsLCView(JWTAuthApiView):
    def get(self, request, tenantName, *args, **kwargs):
        team = team_services.get_tenant_by_tenant_name(tenantName)
        if not team:
            result = general_message(400, "team id null", u"团队不存在")
            return Response(result, status=400)
        tenant_regions = region_services.get_region_info_by_tenant_name(team)
        region_list = region_services.conver_regions_info(tenant_regions, check_status="yes", level="private")
        result = general_message(200, "success", u"查询成功", list=region_list)
        return Response(result, status=result["code"])
    ...
    def post(self, request, tenantName, *args, **kwargs):
        ...
        tenant_regions = region_services.get_region_info_by_tenant_name(team)
        region_list = region_services.conver_regions_info(tenant_regions, check_status="yes", level="private")
        return Response(general_message(200, "success", u"团队添加数据中心成功", list=region_list))


class TeamRegionsDView(JWTAuthApiView):
    def delete(self, request, tenantName, region_name, *args, **kwargs):
        team = team_services.get_tenant_by_tenant_name(tenantName)
        if not team:
            result = general_message(400, "team id null", u"团队不存在")
            return Response(result, status=400)
        ...
        if has_running_service:
            result = general_message(
                400, "this team region has running service ",
                u"当前团队的数据中心存在运行中的组件,不能卸载")
            return Response(result, status=400)
        tenant_region = region_repo.get_team_region_by_tenant_and_region(team.tenant_id, region_name)
        tenant_region.is_active = 0
        tenant_region.save()
        return Response(general_message(200, "success", "团队卸载数据中心成功"))
