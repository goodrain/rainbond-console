    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not check_uuid:
                return Response(
                    general_message(
                        400, "params error", "the field 'check_uuid' is required"
                    ),
                    status=400)

            code, msg, msg_show, services = multi_app_service.list_services(
                self.response_region, self.tenant, check_uuid)
            if code != 200:
                result = general_message(code, msg, msg_show)
            else:
                result = general_message(
                    code, "successfully entered the multi-service creation process",
                    "成功进入多服务创建流程",
                    list=services)
...
def validate_request(item, name):
    if not item:
        return Response(
            general_message(
                400, "params error", "the field '" + name + "' is required"
            ),
            status=400)
    return None
