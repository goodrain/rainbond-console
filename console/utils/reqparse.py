...
def parse_argument(request, key, default=None, value_type=str, required=False, error=''):
    ...
    :return: key's value
    """
    if value_type not in (int, str, list):
        raise TypeError("value_type not in (int, str, list)")
    ...
    value = get.getlist(key, default=default) if value_type is list else get.get(
        key, default=default)
