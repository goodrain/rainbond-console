...
class DeployRepo(object):
    def get_deploy_relation_by_service_id(self, service_id):
        deploy = DeployRelation.objects.get(service_id=service_id).secret_key
        if not deploy:
            ...
            deploy = DeployRelation.objects.create(service_id=service_id, secret_key=secret_key).secret_key
            return deploy
