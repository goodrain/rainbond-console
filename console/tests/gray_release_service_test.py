...
class GrayReleaseRouteUpdateTests(TestCase):
    ...
    def test_update_apisix_route_weights_keeps_service_alias_and_port_in_query(self):
        ...
        original_port = Obj(k8s_service_name="test", container_port=80)
        port_filters = [
            mock.Mock(first=mock.Mock(return_value=new_port)),
            mock.Mock(first=mock.Mock(return_value=None)),
            mock.Mock(first=mock.Mock(return_value=original_port)),
        ]
