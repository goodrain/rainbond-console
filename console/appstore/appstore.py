...
class AppStore(object):
    ...
    def get_plugins_apps(self, store, query, query_all, page=1, page_size=10):
        store_client = get_market_client(store.access_key, store.url)
        data = store_client.get_app_plugin_list(page=page,
                                                page_size=page_size,
    ...
    def get_apps(self, store, query, query_all, page=1, page_size=10):
        store_client = get_market_client(store.access_key, store.url)
        data = store_client.get_user_app_list(page=page,
                                              page_size=page_size,
                                              market_domain=store.domain,
                                              query=query,
                                              query_all=query_all)
    ...
    def get_apps_templates(self, store, query, query_all, page=1, page_size=10):
        store_client = get_market_client(store.access_key, store.url)
        data = store_client.get_app_temp_list(page=page,
                                              page_size=page_size,
                                              market_domain=store.domain,
                                              query=query,
                                              query_all=query_all)
    ...
    def get_app_versions(self, store, app_id, query_all=False):
        store_client = get_market_client(store.access_key, store.url)
        data = store_client.get_user_app_versions(app_id=app_id,
                                                  market_domain=store.domain,
                                                  query_all=query_all,
    ...
    def get_app_version(self, store, app_id, version, for_install=False, get_template=False):
        store_client = get_market_client(store.access_key, store.url)
        data = store_client.get_user_app_version_detail(app_id=app_id,
                                                        version=version,
                                                        market_domain=store.domain,
