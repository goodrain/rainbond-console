...
class AppPortService(object):
    def check_port(self, service, container_port):
        port = port_repo.get_service_port_by_port(
    ...
    def add_service_port(self,
        ...
        logger.debug('======tenant_service_ports======>{0}'.format(
            type(tenant_service_ports)))
    ...
    def get_service_ports(self, service):
        if service:
            return port_repo.get_service_ports(service.tenant_id,
                                               service.service_id)

    def get_service_port_by_port(self, service, port):
        if service:
            return port_repo.get_service_port_by_port(service.tenant_id,
    ...
    def delete_port_by_container_port(self, tenant, service, container_port):
        ...
        if len(service_domain) > 1 or len(
                service_domain) == 1 and service_domain[0].type != 0:
            return 412, u"该端口有自定义域名，请先解绑域名", None

        port_info = port_repo.get_service_port_by_port(
            tenant.tenant_id, service.service_id, container_port)
        if not port_info:
        ...
        if service.create_status == "complete":
            # 删除数据中心端口
            region_api.delete_service_port(
                service.service_region, tenant.tenant_name,
                service.service_alias, container_port, tenant.enterprise_id)
        ...
        env_var_service.delete_env_by_container_port(tenant, service,
                                                     container_port)
        # 删除端口
        port_repo.delete_serivce_port_by_port(
            tenant.tenant_id, service.service_id, container_port)
        # 删除端口绑定的域名
    ...
    def disable_service_when_delete_port(self, tenant, service,
        ...
        probe = probe_repo.get_service_probe(
            service.service_id).filter(is_used=True).first()
        if probe and container_port == probe.port:
            probe.is_used = False
            try:
                probe_service.update_service_probea(
                    tenant=tenant, service=service, data=probe.to_dict())
    ...
    def manage_port(self, tenant, service, region_name, container_port, action,
                    protocol, port_alias):
        if port_alias:
            port_alias = str(port_alias).strip()
        region = region_repo.get_region_by_region_name(region_name)
        code, msg = self.__check_params(action, protocol, port_alias,
                                        service.service_id)
        if code != 200:
            return code, msg, None
        deal_port = port_repo.get_service_port_by_port(
            tenant.tenant_id, service.service_id, container_port)
        if action == "open_outer":
            ...
            code, msg = self.__change_port_alias(tenant, service, deal_port,
                                                 port_alias)
        new_port = port_repo.get_service_port_by_port(
            tenant.tenant_id, service.service_id, container_port)
        if code != 200:
            return code, msg, None
    ...
    def __open_inner(self, tenant, service, deal_port):
        ...
        env_prefix = deal_port.port_alias.upper() if bool(
            deal_port.port_alias) else service.service_key.upper()
        # 添加环境变量
        code, msg, data = env_var_service.add_service_env_var(
            tenant,
            service,
        ...
        if service.create_status == "complete":
            body = region_api.manage_inner_port(
                service.service_region, tenant.tenant_name,
                service.service_alias, deal_port.container_port, {
                    "operation": "open",
                    "enterprise_id": tenant.enterprise_id
    ...
    def __change_protocol(self, tenant, service, deal_port, protocol):
        ...
        if service.create_status == "complete":
            ...
            region_api.update_service_port(
                service.service_region, tenant.tenant_name,
                service.service_alias, {
                    "port": [body],
                    "enterprise_id": tenant.enterprise_id
    ...
    def __change_port_alias(self, tenant, service, deal_port, new_port_alias):
        ...
        envs = env_var_service.get_env_by_container_port(
            tenant, service, deal_port.container_port)
        for env in envs:
            old_env_attr_name = env.attr_name
            new_attr_name = new_port_alias + env.attr_name.replace(
                old_port_alias, '')
            env.attr_name = new_attr_name
            if service.create_status == "complete":
                region_api.delete_service_env(
                    service.service_region, tenant.tenant_name,
                    service.service_alias, {
    ...
    def get_access_info(self, tenant, service):
        ...
        access_type, data = self.__handle_port_info(
            tenant, service, unopened_port, http_outer_port, http_inner_port,
            stream_outer_port, stream_inner_port)
        return access_type, data

    def __handle_port_info(self, tenant, service, unopened_port,
                           http_outer_port, http_inner_port, stream_outer_port,
                           stream_inner_port):
        # 有http对外访问端口
        if http_outer_port:
            ...
            for p in http_outer_port:
                port_dict = p.to_dict()
                port_dict["access_urls"] = self.__get_port_access_url(
                    tenant, service, p.container_port)
        ...
        if stream_outer_port:
            ...
            for p in stream_outer_port:
                ...
                associate_info = self.get_port_associated_env(
                    tenant, service, p.container_port)
                port_dict = p.to_dict()
        ...
        if stream_inner_port:
            ...
            for p in stream_inner_port:
                port_dict = p.to_dict()
                env_list = self.get_port_associated_env(
                    tenant, service, p.container_port)
    ...
    def __get_stream_outer_url(self, tenant, service, port):
        ...
        if region:
            service_tcp_domain = tcp_domain.get_service_tcpdomain(
                tenant.tenant_id, region.region_id, service.service_id,
                port.container_port)
    ...
    def get_port_associated_env(self, tenant, service, port):
        ...
        env_var = env_var_service.get_env_by_container_port(
            tenant, service, port)
        env_list = [{
            "name": e.name,
    ...
    def get_team_region_usable_tcp_ports(self, tenant, service):
        services = service_repo.get_service_by_region_and_tenant(
            service.service_region, tenant.tenant_id)
        current_service_tcp_ports = port_repo.get_service_ports(
            tenant.tenant_id,
            service.service_id).filter(is_outer_service=True).exclude(
                protocol__in=("http", "https"))
        lb_mapping_ports = [
            p.lb_mapping_port for p in current_service_tcp_ports
        ]
        service_ids = [s.service_id for s in services]
        res = port_repo.get_tcp_outer_opend_ports(service_ids).exclude(
            lb_mapping_port__in=lb_mapping_ports)
        return res

    def change_lb_mapping_port(self, tenant, service, container_port,
                               new_lb_mapping_port, mapping_service_id):
        data = {"change_port": new_lb_mapping_port}
        region_api.change_service_lb_mapping_port(
            service.service_region, tenant.tenant_name, service.service_alias,
            container_port, data)

        current_port = port_repo.get_service_port_by_port(
            tenant.tenant_id, service.service_id, container_port)
        exchange_port_info = port_repo.get_service_port_by_lb_mapping_port(
            mapping_service_id, new_lb_mapping_port)
        if not current_port:
