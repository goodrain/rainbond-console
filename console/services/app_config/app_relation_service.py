...
class AppServiceRelationService(object):
    ...
    def get_undependencies(self, tenant, service):
        ...
        not_dependencies = []
        dep_services = dep_relation_repo.get_service_dependencies(
        ...
        for s in services:
            ...
            open_inner_services = port_repo.get_service_ports(
                tenant.tenant_id, s.service_id).filter(is_inner_service=True)
    ...
    def patch_add_dependency(self, tenant, service, dep_service_ids):
        ...
        for dep_id in dep_service_ids:
            code, msg, relation = self.add_service_dependency(
                tenant, service, dep_id)
            if code != 200:
                return code, msg
