...
class GroupAppBackupService(object):
    ...
    def backup_group_apps(self, tenant, user, region_name, group_id, mode, note, force=False):
        ...
        try:
            ...
            if "some services do not exist in need backup services" in e.message["body"].get("msg", ""):
                raise ServiceHandleException(msg="backup failed", msg_show="某些组件不存在，无法备份，请检查是否所有组件都已成功创建")
