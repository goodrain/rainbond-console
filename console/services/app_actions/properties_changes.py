...
class PropertiesChanges(object):
    ...
    def get_property_changes(self, eid, version):
        ...
        dep_volumes = self.dep_volumes_changes(
            app.get("mnt_relation_list", []))
        if dep_volumes:
    ...
    def env_changes(self, envs):
        ...
        add_env = [
            env for env in envs
            if exist_envs_dict.get(env["attr_name"], None) is None
        ]
        if not add_env:
            return None
        return {"add": add_env}
    ...
    def dep_services_changes(self, dep_uuids):
        ...
        new_dep_services = service_repo.list_by_svc_share_uuids(
            group_id, dep_uuids)
        add = [
            svc for svc in new_dep_services
            if svc.service_id not in service_ids
        ]
        if not add:
            return None
        return {
    ...
    def plugin_changes(self, new_plugins):
        old_plugins, _ = app_plugin_service.get_plugins_by_service_id(
            self.service.service_region, self.service.tenant_id,
            self.service.service_id, "")
        ...
        for new_plugin in new_plugins:
            if new_plugin["plugin_key"] in old_plugin_keys:
                continue
            new_plugin["plugin_alias"] = plugin_names.get(
                new_plugin["plugin_key"], new_plugin["plugin_key"])
    ...
    def dep_volumes_changes(self, new_dep_volumes):
        ...
        old_dep_volumes = mnt_repo.get_service_mnts(self.service.tenant_id,
                                                    self.service.service_id)
        olds = {
            key(item.dep_service_id, item.mnt_name): item
            for item in old_dep_volumes
        }

        tenant_service_volumes = volume_repo.get_service_volumes(
            self.service.service_id)
        ...
        for new_dep_volume in new_dep_volumes:
            ...
            if olds.get(
                ...
                logger.debug("ignore dep volume: {}; dep volume exist.".format(
                    key(dep_service["service_id"],
                        new_dep_volume["mnt_name"])))
                continue

            code, msg = volume_service.check_volume_path(
                self.service, new_dep_volume["mnt_dir"], local_path=local_path)
            if code != 200:
                logger.warning(
                    "service id: {}; path: {}; invalid volume: {1}".format(
                        self.service.service_id, new_dep_volume["mnt_dir"],
                        msg))
...
def has_changes(changes):
    ...
    def beta(x):
        return x and (x.get("add", None) or x.get("del", None)
                      or x.get("upd", None))
