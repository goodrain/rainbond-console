...
class TenantStopView(APIView, RegionOperateMixin):
    ...
    def post(self, request, tenantId, format=None):
        ...
        try:
            ...
            tenant_source_region = TenantRegionInfo.objects.get(
                tenant_id=tenantId, region_name=source_region)
            tenant_dest_region, created = TenantRegionInfo.objects.get_or_create(
                tenant_id=tenantId, region_name=dest_region)
            if created or not tenant_dest_region.is_active:
                logger.info("tenant.move", "init region {0} for tenant {1} for move".format(
                    dest_region, tenant.tenant_name))
                success = self.init_for_region(
                    dest_region, tenant.tenant_name, tenant.tenant_id)
        ...
        if tenant_dest_region.is_active:
            report = self.stop_and_recreate_services(
    ...
    def stop_and_recreate_services(self, tenant, source_region, dest_region):
        ...
        moving_services = TenantServiceInfo.objects.filter(
            tenant_id=tenant.tenant_id, service_region=source_region)

        try:
            # 关闭源机房的全部服务
            for service in moving_services:
                regionClient.stop(source_region, service.service_id)
                logger.info("tenant.move", "stop service {0}".format(
                    service.service_id))

            # 在目标机房生成服务以及环境变量
            for service in moving_services:
                baseService.create_region_service(
                    service, tenant.tenant_name, dest_region, do_deploy=False)
                logger.info("tenant.move", "copy service {0} to region {1}".format(
                    service.service_id, dest_region))
                baseService.create_service_env(
                    tenant.tenant_id, service.service_id, dest_region)
                logger.info("tenant.move", "copy service env of {0} to region {1}".format(
                    service.service_id, dest_region))
                report['moving_services'].append(service.service_id)

            # 在目标机房生成服务依赖关系
            ids = map(lambda s: s.service_id, moving_services)
            service_relations = TenantServiceRelation.objects.filter(
                service_id__in=ids)
            for relation in service_relations:
                ...
                regionClient.createServiceDependency(
                    dest_region, service_id, json.dumps(task))
                logger.info("tenant.move", "copy relation {0} to region {1}".format(
                    task, dest_region))
        except Exception, e:
            logger.exception("tenant.move", e)
...
class TenantStartView(APIView):
    ...
    def post(self, request, tenantId, format=None):
        ...
        logger.info("tenant.move", "start all services on region {0} for tenant {1}".format(
            dest_region, tenantId))

        try:
            tenant = Tenants.objects.get(tenant_id=tenantId)
            tenant_dest_region = TenantRegionInfo.objects.get(
                tenant_id=tenantId, region_name=dest_region)
            if not tenant_dest_region.is_active:
                msg = "region {0} for tenant {1} is not active".format(
                    dest_region, tenant.tenant_name)
    ...
    def start_services(self, tenant, source_region, dest_region):
        ...
        try:
            ...
            service_relations = TenantServiceRelation.objects.filter(
                service_id__in=ids)
            dep_service_ids = set(
                [s.dep_service_id for s in service_relations])
            start_at_end_ids = set(ids).difference(dep_service_ids)

            report.update({"dep_services": dep_service_ids,
                           "starts_services": start_at_end_ids})
    ...
    def start(self, service_id, region):
        logger.info("tenant.move", "region {0}, try to start service {1}".format(
            region, service_id))
        service = TenantServiceInfo.objects.get(service_id=service_id)
        logger.info("tenant.move",
                    "service category is {0}".format(service.category))
        if service.category == 'application':
            ...
            logger.info(
                "tenant.move", "call regionapi for build_service {0}".format(service_id))
        else:
            service.deploy_version = make_deploy_version()
            service.save()
            ...
            logger.info(
                "tenant.move", "call regionapi for deploy_service {0}".format(service_id))
        logger.info("tenant.move", "region {0}, started service {1}".format(
            region, service_id))
...
class TenantFollowUpView(APIView):
    ...
    def post(self, request, tenantId, format=None):
        ...
        logger.info(
            "tenant.move", "do follow-up works on region {0} for tenant {1}".format(dest_region, tenantId))
        ...
        if tenant_dest_region.is_active:
            report = self.do_follow_up_works(
                tenant, source_region, tenant_dest_region)
            if report['ok']:
                return Response(report, status=200)
            else:
                return Response(report, status=500)
        ...
        try:
            for service in moving_services:
                try:
                    ...
                    pass
                logger.info("tenant.move", "delete service {0} from source region {1}".format(
                    service.service_id, source_region))
                ...
                service_domains = ServiceDomain.objects.filter(
                ...
                for domain in service_domains:
                    ...
                    regionClient.addUserDomain(
                        tenant_dest_region.region_name, json.dumps(task))

            if tenant_dest_region.service_status != 1:
                regionClient.pause(
                    tenant_dest_region.region_name, tenant.tenant_id)
