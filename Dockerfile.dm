...
# Build the frontend from the normal repository context. The ignored Dameng
# driver bundle is supplied separately through the named `dameng` context.
...
COPY --from=dameng bin/libdmdpi.so /opt/dameng/bin/libdmdpi.so
COPY --from=dameng drivers/python/dmPython/ /opt/dameng/drivers/python/dmPython/
COPY --from=dameng drivers/python/dmDjango/dmDjango3.0/ /opt/dameng/drivers/python/dmDjango/dmDjango3.0/
