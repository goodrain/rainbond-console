	...
	def addServicePort(self, service, is_init_account, container_port=0, protocol='', port_alias='',
		...
		try:
			...
			if is_init_account:
				...
				self.saveServiceEnvVar(service.tenant_id, service.service_id, container_port, u"用户名",
									   env_prefix + "_USER", "admin", False, scope="both")
				self.saveServiceEnvVar(service.tenant_id, service.service_id, container_port, u"密码",
									   env_prefix + "_PASS", password, False, scope="both")
			port.save()
		except Exception, e:
			logger.exception(e)
