gWidget.define('setGoodrainGitUser', {
    ...
    _defaultOption : {
    	_tpl:'<form class="form-horizontal" style="padding:40px 20px;margin-left:auto;margin-right:auto;color:#303030">'+
			  '<div class="form-group">'+
			    ...
			    '<div class="col-sm-8">'+
			     '<input type="text" class="email form-control" placeholder="请输入一个邮箱地址作为您的好雨Git账号">'+
			    '</div>'+
			  '</div>'+
			  '<div class="form-group">'+
    ...
    check: function(){
    	...
    	if(!data.email){
    		gWidget.Message.warning("请输入好雨Git账号");
    		return false;
    	}

    	if(!/^(.+)@([a-zA-Z0-9]+)((\.)([a-zA-Z]+)){1,2}$/i.test(data.email)){
    		gWidget.Message.warning("账号格式不正确，只能为邮箱格式");
    		return false;
    	}
