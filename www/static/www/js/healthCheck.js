...
//当启动时状态变化时
function onStartStatusChange(isUsed) {
 ...
 //当运行时状态变化时
function onRunStatusChange(isUsed) {
    if (isUsed === true) {
...
*/
function getHealthCheckInfo(tenantName, serviceAlias, mode){
    var url = '/ajax/'+tenantName+'/'+serviceAlias+'/probe?mode='+mode;
...
/*
  激活/禁用健康监测　
*/
function activeAndDisableHealthCheck(tenantName, serviceAlias, probe_id){
  var url = '/ajax/'+tenantName+'/'+serviceAlias+'/probe/'+probe_id+'/update_used';
  return http({
       url:url,
       type:'POST'
  })
...
gWidget.define('HealthCheckInfo', {
  _defaultOption:{
      tpl:'<div style="overflow:hidden;">'+
            '<form class="form-horizontal">'+
              ...
              '<div class="form-group">'+
               ...
               '<div class="col-sm-8">'+
                  '<p class="form-control-static js-port">5000</p>'+
                '</div>'+
              '</div>'+
              '<div class="form-group">'+
  ...
  setValue:function(data) {
     ...
     this.$headers.html(this.data.http_header);
...
gWidget.define('HealthCheckForm', {
  _defaultOption:{
      tpl:'<div class="edit" style="overflow:hidden;">'+
            '<form class="form-horizontal">'+
              ...
              '<div class="form-group  js-headers-group">'+
                ...
                '<div class="col-sm-8 js-header-wrap">'+
                  '<div class="form-inline js-header-row">'+
                    ...
                    '</div>'+
                    '<div class="form-group">'+
                      '<input type="text" name="value" style="width:90px" class="form-control" placeholder="value">'+
                    '</div>'+
                    '<div class="form-group">'+
              ...
              '<div class="form-group js-path-group">'+
                ...
                '<div class="col-sm-8">'+
                  '<input type="text" class="js-path form-control" name="path" style="width:260px"  placeholder="请填写检测路径" />'+
                '</div>'+
              '</div>'+
  ...
  renderPort:function(port){
     ...
     for(var i=0,len=ports.length;i<len;i++){
        html.push('<option value="'+ports[i]+'">'+ports[i]+'</option>')
     }
  ...
  gWidget.define('viewAndEditStartHealthCheck', {
    _defaultOption:{
        tpl:'<div></div>',
        width:'700px',
        height: '500px',
    ...
    _init:function(option){
      ...
      getHealthCheckInfo(
        ...
        this.option.mode
          ...
          self.ok = true;
          self.changeToView();
          self.healthCheckInfo.setValue(data.bean);
          self.healthCheckForm.setValue(data.bean);
           ...
           if(data){
               if(data.code === 404){
                   self._create();
                   self.bind();
                   self.healthCheckForm.setValue({
                    mode:self.option.mode,
                    port: self.option.port.split(',')[0],
                    scheme: 'tcp'
                 });  
                 self.changeToAdd();
               }else{
                  gWidget.Message.danger(data.msgcn);
               }
           }
      })
