    		...
    		html.push('<div class="panel panel-primary">');
    			html.push('<div class="panel-heading">');
    				...
    				html.push('<div class="user btns">');
    					...
    					html.push('<a href="javascript:;" class="ajax_log_new">查看详情</a>');
    			...
    			if(appActionLogUtil.canRollback(log)){
					html.push('<div class="version-info">');
						html.push('<span class="pull-left">代码信息：'+appActionLogUtil.getCommitLog(log)+'</span>');
		...
		renderPlugin: function(){
			...
			if(!this.plugins.length){
				...
				for(var i=0;i<this.plugins.length;i++){
					...
					html.push('<li class="'+(!item.is_switch ? 'disabled' : '')+'"><a href="/apps/'+this.tenantName+'/'+this.serviceAlias+'/detail/?fr=plugin&plugin_id='+item.plugin_id+'">'+item.plugin_info.plugin_alias+'</a></li>')
				}
				html.push('</ul>')
		...
		loadMonitorData: function(){
			$.when(
				...
				if(result1 && result1.length){
	        		...
	        		$('.app_requesttime').html(res.toFixed(2));
	        		
	        	}
	        	if(result2 && result2.length){
	        		var res = Number(result2[0].value[1]) || 0;
	        		$('.app_request').html(res.toFixed(2));
