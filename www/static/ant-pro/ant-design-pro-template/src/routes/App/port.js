              ...
              >
                  {
                    getFieldDecorator('private_key', {
                       rules:[{required: true, message: '请添加key'}]
                    })(
              ...
              >
                  {
                    getFieldDecorator('certificate', {
                       rules:[{required: true, message: '请添加证书'}]
                    })(
...
class AddDomain extends PureComponent {
    ...
    var visitType = this.props.form.getFieldValue("protocol");
    if (visitType == 'http') {
        callback();
        return;
      }
      ...
      return (
          ...
          >
            ...
            <Form onSubmit={this.handleSubmit}>
              ...
              >
                  {
                    getFieldDecorator('domain', {
                       rules:[{required: true, message: '请添加域名'}]
                    })(
              ...
              <FormItem
                style={{display:protocol == 'http' ? 'none': ''}}
                {...formItemLayout}
                label="选择证书"
              >
                  {getFieldDecorator('certificate_id', {
                    initialValue:'',
                      ...
                      <p>无可用证书？ <a  onClick={()=>{this.props.onCreateKey()}} href="javascript:;">去新建</a></p>
              </FormItem>
            </Form>
...
class ChangeProtocol extends PureComponent {
   ...
   render(){
      ...
      return (
          <Form layout="inline" style={{position: 'relative', top: -8}}>
             <FormItem>
               <Select onChange={this.onChange} size="small" value={this.state.value} style={{width: 80}}>
                 <Option value="http">http</Option>
                 <Option value="tcp">tcp</Option>
                 <Option value="udp">udp</Option>
                 <Option value="mysql">mysql</Option>
...
class Port extends PureComponent {
    ...
    handleOuterChange =(val) => {
      this.props.onOuterChange(this.props.port.container_port, val);
    }
    handleInnerChange =(val) => {
      this.props.onInnerChange(this.props.port.container_port, val);
    }
    showEditProtocol = () => {
    ...
    onSubmitProtocol = (val) => {
      this.props.onChangeProtocol && this.props.onChangeProtocol(this.props.port.container_port, val);
      ...
      return (
           <Card
             ...
             title={
              ...
              <DescriptionList>
                <Description term="端口">{port.container_port}</Description>
                <Description term="协议">
                   {
                      this.state.editProtocol ? 
                      <ChangeProtocol protocol={port.protocol} onSubmit={this.onSubmitProtocol} onCancel={this.cancelEditProtocol} />
                      :
                      <div>
                          {port.protocol} <a onClick={this.showEditProtocol} href="javascript:;"><Icon type="edit" /></a>
                      </div>
                   }
                  
                    
                </Description>
                <Description term=""><a href="javascript:;" onClick={()=>{this.props.onDeletePort(port.container_port)}} style={{float: 'right', fontWeight: 'normal', fontSize: 14, flaot: 'right'}}>删除端口</a></Description>
              </DescriptionList>
             }
           >
             <DescriptionList style={{ marginBottom: 24 }}>
              <Description term="内部访问"><Switch checked={appPortUtil.isOpenInner(port)} onChange={this.handleInnerChange} /></Description>
              <Description term="服务地址">{innerUrl ? innerUrl : '-'}</Description>
              <Description term="使用别名"><div>{showAlias} <a href="javascript:;" onClick={() => {this.props.onEditAlias(port)}}><Icon type="edit" /></a></div></Description>
            </DescriptionList>
            <DescriptionList style={{ marginBottom: 24 }}>
              <Description term="外部访问"><Switch checked={appPortUtil.isOpenOuter(port)} onChange={this.handleOuterChange} /></Description>
              <Description term="访问地址">{outerUrl ? <a href={outerUrl} target={outerUrl} target="_blank">{outerUrl}</a> : '-'}</Description>

               <Description term="绑定域名" style={{display: appPortUtil.canBindDomain(port) ? '' : 'none'}}>
                  <div className={styles.domainList}>
                    {
                      domains.map((domain) => {
                          return  <div>
                                    ...
                                    }} className={styles.removePort} href="javascript:;"><Icon type="close" /></a>
                                  </div>
                      })
                    }
                  </div>
                  <Button onClick={()=>{this.props.onAddDomain(port.container_port)}} size="small" type="dashed"><Icon type="plus" /></Button>
              </Description>
...
export default class Index extends PureComponent {
  ...
  handleSubmitProtocol = (port, protocol) => {
      const { dispatch } = this.props;
      dispatch({
  ...
  render() {
    ...
    return (
        ...
        <Row>
          <Col span={12}>
            <Alert showIcon message="域名配置信息发生变化后需要重启应用才能生效" type="info" style={{marginBottom: 24}} />
          </Col>
          <Col span={12} style={{textAlign: 'right'}}><Button onClick={this.showAddPort} type="primary"><Icon type="plus" />添加端口</Button></Col>
        </Row>

        {
          !ports.length ? 
          <Card>
              <p style={{marginTop: 100, marginBottom:100, fontSize: 20, textAlign: 'center'}}>如需要提供访问服务，请<a  onClick={this.showAddPort} href="javascript:;">添加端口</a></p>
          </Card>
          :
          ports.map((port) => {
              return <Port onEditAlias={this.showEditAlias} onInnerChange={this.handleInnerChange} onOuterChange={this.handleOuterChange} port={port} onAddDomain={this.onAddDomain} onDeleteDomain={this.handleDeleteDomain} onDeletePort ={this.handleDeletePort} onChangeProtocol={this.handleSubmitProtocol} />
          })
        }
        {this.state.showDeletePort && <ConfirmModal title="端口删除" desc="确定要删除此端口吗？" subDesc="此操作不可恢复" onOk={this.handleSubmitDeletePort} onCancel={this.cancalDeletePort} />}
        {this.state.showDeleteDomain && <ConfirmModal title="域名解绑" desc="确定要解绑此域名吗？" subDesc={this.state.showDeleteDomain.domain} onOk={this.handleSubmitDeleteDomain} onCancel={this.cancalDeleteDomain} />}
        {this.state.showAddPort && <AddPort onCancel={this.onCancelAddPort}  onOk={this.handleAddPort} />}
        {this.state.showAddDomain && <AddDomain  certificates={certificates||[]} onCreateKey={this.handleCreateKey} onOk={this.handleAddDomain} onCancel={this.onCancelAddDomain} />}
        {this.state.showAddKey && <AddKey onOk={this.handleSubmitKey} onCancel={this.cancelCreateKey} />}
        {this.state.showEditAlias && <EditAlias port={this.state.showEditAlias}  onOk={this.handleEditAlias} onCancel={this.hideEditAlias} />}
        </Fragment>
    );
  }
}
