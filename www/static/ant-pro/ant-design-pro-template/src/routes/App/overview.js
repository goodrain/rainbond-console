...
import { Row, Col, Card, Form, Button, Icon, Menu, Dropdown, Tooltip} from 'antd';
...
class LogItem extends PureComponent {
  ...
  componentDidMount(){
    ...
    if(data){
        if(this.ref){
            ...
            this.ref.querySelector('.actionresultcn').innerHTML = (appAcionLogUtil.getActionResultCN(data));
            if(appAcionLogUtil.isFail(data)){
              this.ref.querySelector('.action-error-msg').innerHTML = '(' + appAcionLogUtil.getFailMessage(data) + ')';
            }
  ...
  createSocket(){
     ...
     this.socket = new LogSocket({
        ...
        onMessage: (data) => {
          ...
          logs.unshift(data);
          this.setState({logs: [].concat(logs)});
        },
  ...
  open = () =>  {
     this.setState({opened: true, logType: 'info'}, ()=>{
        this.loadLog(); 
     })
  }
  close = () =>  {
  ...
  handleRollback = () => {
       this.context.appRolback(appAcionLogUtil.getRollbackVersion(this.props.data));
  }
  render(){
     ...
     return (
        <div ref={this.saveRef} className={styles.logItem + ' ' + this.getResultClass()}>
            ...
            <div className={styles.logItemMain}>
              <div className={styles.hd}>
                ...
                <div className={styles.btns}>
                    ...
                    {!opened ? <span onClick={this.open} className={styles.btn}>查看详情</span> : ''}
                    {(opened) ? <span onClick={this.close} className={styles.btn}>收起</span> : ''}
                </div>
              </div>
              {
                 ...
                 <div className={styles.codeVersion}>
                  ...
                  <div className={styles.versionAuthor}>#{appAcionLogUtil.getCodeVersion(data)} by {appAcionLogUtil.getCommitUser(data)}</div>
                </div>
                :''
              }
              
              <ButtonGroup  style={{display: this.showLogType()}} size="small" className={styles.logTypeBtn}>
                  <Button onClick={()=>{this.changeLogType('info')}} className={logType === 'info' ? 'active': ''} type={'dashed'}>Info日志</Button>
                  <Button onClick={()=>{this.changeLogType('debug')}} className={logType === 'debug' ? 'active': ''} type={'dashed'}>Debug日志</Button>
                  <Button  onClick={()=>{this.changeLogType('error')}} className={logType === 'error' ? 'active': ''} type={'dashed'}>Error日志</Button>
              </ButtonGroup>
              <div style={{height: this.getLogContHeight(), maxHeight: 500, overflowY: 'auto'}} className={styles.logContent + ' logs-cont'}>
              {status === 'ing' && <LogProcress resover={true} onClose={this.onClose} onComplete={this.onComplete} onSuccess={this.onSuccess} onFail={this.onFail} socketUrl={this.getSocketUrl()} eventId={data.event_id} /> }
                {
                   (logs||[]).map((item)=>{
                      return <p><span style={{marginRight: 10}}>{dateUtil.format(item.time, 'hh:mm:ss')}</span><span>{item.message}</span></p>;
                   })
                }
              </div>
            </div>
          ...
          <div className={styles.logs}>
            {
               list.map((item)=>{
                  return (
                      <LogItem  key={item.event_id} appAlias={this.props.appAlias} data={item} />
                  )
               })
            }
          </div>
...
export default class Index extends PureComponent {
  ...
  componentWillUnmount(){
     this.mounted = false;
     this.props.dispatch({
      type: 'appControl/clearDisk'
    })
     this.props.dispatch({
      type: 'appControl/clearMemory'
    })
    this.props.dispatch({
      type: 'appControl/clearRequestTime'
    })
    this.props.dispatch({
      type: 'appControl/clearRequestTimeRange'
    })
    this.props.dispatch({
      type: 'appControl/clearRequest'
    })
    this.props.dispatch({
      type: 'appControl/clearRequestRange'
    })
  }
  fetchAppDisk(){
    this.props.dispatch({
  ...
  loadLog = (append) => {
      ...
      getActionLog({
        ...
        team_name: globalUtil.getCurrTeamName()
          ...
          if(data){
              if(!append){
                this.setState({hasNext: data.has_next, logList: (data.list||[])})
              }else{
                this.setState({hasNext: data.has_next, logList: (this.state.logList).concat(data.list||[])})
              }
          }
  ...
  onAction = (actionLog) => {
    ...
    this.setState({logList: [actionLog].concat(this.state.logList)});
  }
  handleNextPage = () => {
  ...
  render() {
    ...
    const {logList, hasNext} = this.state;
     ...
     <Row gutter={24}>
      <Col {...topColResponsiveProps}>
          <ChartCard
            bordered={false}
            title="应用状态"
            footer={<span className={styles.statuscn}>{status.status_cn || '-'}</span>}
          >
       ...
       <Col {...topColResponsiveProps}>
          {
            applUtil.isInstalledPowerPlugin(appDetail) ? 
            <ChartCard
              bordered={false}
              title="平均响应时间"
              action={
              ...
              contentHeight={46}
            >
              <MiniArea color="#975FE4"  data={monitorDataUtil.queryRangeTog2(this.props.requestTimeRange)} />
            </ChartCard>
            :
            <ChartCard
                bordered={false}
              ...
              >
                <div style={{textAlign: 'center',position: 'relative', top: -10}}>
                  <p>暂无安装性能分析插件</p>
                   <Link to={'/app/'+this.props.appAlias+ '/plugin'}>去安装</Link>
              </div>
            </ChartCard>
          }
        ...
        <Col {...topColResponsiveProps}>

          {
            ...
            <ChartCard
              ...
              contentHeight={46}
            >
              <MiniArea  color="#4593fc"  data={monitorDataUtil.queryRangeTog2(this.props.appRequestRange)} />
            </ChartCard>
            :
          ...
          >
           <div className={styles.charContent}>
                <p className={styles.charContentTit}>
                  {numeral(monitorDataUtil.queryTog2(this.props.appMemory)).format('0,0')} <span className={styles.sub}>MB 内存</span>
                </p>

                <p className={styles.charContentTit}>
                  {numeral(monitorDataUtil.queryTog2(this.props.appDisk, (v)=>{return v/1024})).format('0,0')} <span className={styles.sub}>MB 磁盘</span>
                </p>
     ...
     <Row gutter={24}>
        <Col xs={24} xm={24} md={24} lg={24} xl={24}>
          <Card
            ...
            title="操作日志"
          >
            <LogList appAlias={this.props.appAlias} list={logList||[]} />
            {
              this.state.hasNext && <p style={{textAlign: 'center', fontSize: 30}}><Icon style={{cursor: 'pointer'}} onClick={this.handleNextPage} type="down" /></p>
            }
