...
class AddRelation extends PureComponent {
   ...
   handleSubmit = () => {
      ...
      var res=[];
      res = this.state.selectedRowKeys.map((id) => {
          return {
             id: id,
             path: this.state.localpaths[id]
          }
      })
      res = res.filter((item) => {
          return !!item.path;
   ...
   isDisabled = (data) => {
     return this.state.selectedRowKeys.indexOf(data.dep_vol_id) === -1;
   }
   ...
   render(){
      const rowSelection = {
        onChange: (selectedRowKeys, selectedRows) => {
           this.setState({selectedRowKeys: selectedRows.map((item)=>{
      ...
      return (
        ...
        <Table
          ...
          render: (localpath, data, index)=>{
             return <Input onChange={(e) => {this.handleChange(e.target.value, data)}}  disabled={this.isDisabled(data)} />
          }
        },{
          title:'目标持久化名称',
...
export default class Index extends PureComponent {
  ...
  loadMntList = () => {
      getMnt({
         ...
         page_size: 1000
          ...
          if(data){
              this.setState({mntList: data.list || []})
          }
      })
  }
  handleAddVar = () => {
  ...
  render() {
    ...
    return (
      ...
      <Row>
          <Col span={12}>
            <Alert showIcon message="配置信息发生变化后需要重启应用才能生效" type="info" style={{marginBottom: 24}} />
          </Col>
        </Row>
     <Card
     ...
     >
      <Table 
        ...
        columns={[{
          ...
          dataIndex: 'action',
          render: (val, data) => {
              return <a onClick={()=> {this.onDeleteVolume(data)}} href="javascript:;">删除</a>
          }
        }]}
        dataSource={volumes}
       />
     ...
     >
      ...
      columns={[{
          ...
          dataIndex: 'action',
          render: (val, data) => {
              return <a onClick={() => {this.onDeleteMnt(data)}} href="javascript:;">取消挂载</a>
          }
        }]}
      dataSource={mntList}
       />
       <div  style={{marginTop:10, textAlign: 'right'}}>
        <Button onClick={this.showAddRelation}><Icon type="plus" /> 挂载目录</Button>
       </div>
     </Card>
     {this.state.showAddVar && <AddVolumes onCancel={this.handleCancelAddVar} onSubmit={this.handleSubmitAddVar} data={this.state.showAddVar} />}
     {this.state.showAddRelation && <AddRelation appAlias={this.props.appAlias} onCancel={this.handleCancelAddRelation} onSubmit={this.handleSubmitAddMnt} />}
     {this.state.toDeleteMnt && <ConfirmModal title="取消挂载" desc="确定要取消此挂载目录吗?" onCancel={this.cancelDeleteMnt} onOk={this.handleDeleteMnt} />}
     {this.state.toDeleteVolume && <ConfirmModal title="删除持久化目录" desc="确定要删除此持久化目录吗?" onCancel={this.onCancelDeleteVolume} onOk={this.handleDeleteVolume} />}
     </Fragment>
    );
  }
}
