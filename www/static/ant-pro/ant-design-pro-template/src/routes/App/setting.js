...
import { Row, Col, Card, Form, Button, Icon, Menu, Dropdown, Select, Radio, Table, Modal, Input, Tag} from 'antd';
...
class AddTag extends PureComponent {
   ...
   render(){
      ...
      return (
          ...
          {
             (!tags || !tags.length) ? 
             <div style={{textAlign: 'center'}}>暂无可用标签</div>
             :
             tags.map((tag)=>{
                return <CheckableTag onChange={(checked)=>{this.handleChange(tag.label_id, checked)}} checked={this.isCheck(tag.label_id)}>{tag.label_alias}</CheckableTag>
             })
          }
          </Modal>
      )
  ...
  render(){
    ...
    return(
        ...
        <Form>
          ...
          label="初始化等候时间"
          >
            <span>{appProbeUtil.getInitWaitTime(data)} <span style={{marginLeft: 8}}>秒</span></span>
          </FormItem>
          <FormItem
          {...formItemLayout}
          label="检测监测时间"
          >
            <span>{appProbeUtil.getIntervalTime(data)} <span style={{marginLeft: 8}}>秒</span></span>
          </FormItem>
          <FormItem
          {...formItemLayout}
          label="检测超时时间"
          >
            <span>{appProbeUtil.getTimeoutTime(data)} <span style={{marginLeft: 8}}>秒</span></span>
          </FormItem>
          <FormItem
          {...formItemLayout}
          label="连续成功次数"
...
class EditHealthCheck extends PureComponent {
  ...
  checkPath = (rule, value, callback) => {
    var visitType = this.props.form.getFieldValue("scheme");
    if (visitType == 'tcp') {
        callback();
        return;
      }
          ...
          <Form onSubmit={this.handleSubmit}>
              ...
              >
              {
                getFieldDecorator('port', {
                  initialValue: appProbeUtil.getPort(data)  || (ports.length ? ports[0].container_port : '')
                })(
                  <Select>
                    {
                       ports.map((port) => {
              ...
              >
              {
                getFieldDecorator('path', {
                  ...
                  rules:[{validator: this.checkPath}]
                })(
              ...
              >
              {
                getFieldDecorator('initial_delay_second', {
                  ...
                  rules:[{required: true, message: '请填写初始化等候时间'}]
                })(
              ...
              >
              {
                getFieldDecorator('period_second', {
                  ...
                  rules:[{required: true, message: '请填写检测间隔时间'}]
                })(
                  <Input type="number"  style={{width: '80%'}} />
                )
              }
              ...
              >
              {
                getFieldDecorator('timeout_second', {
                  ...
                  rules:[{required: true, message: '请填写检测超时时间'}]
                })(
                  <Input type="number"  style={{width: '80%'}} />
                )
              }
              ...
              >
              {
                getFieldDecorator('success_threshold', {
                  ...
                  rules:[{required: true, message: '请填写连续成功次数'}]
                })(
                  <Input type="number"  style={{width: '80%'}} />
                )
              }
          ...
          <Form onSubmit={this.handleSubmit}>
              ...
              >
              {
                getFieldDecorator('failure_threshold', {
                  ...
                  rules:[{required: true, message: '请填写连续错误次数'}]
                })(
                  <Input type="number"  style={{width: '80%'}} />
                )
              }
...
class ChangeBranch extends PureComponent {
   ...
   render(){
      ...
      return (
         ...
         <FormItem {...formItemLayout}  label="代码分支">
              <Select onChange={this.handleChange} value={this.state.curr} style={{width: 120}}>
                {
                  branch.map((item)=>{
              ...
              <Button onClick={this.handleSubmit}  style={{marginLeft: 10}} type="primary">确定</Button>
          </FormItem>
      )
...
export default class Index extends PureComponent {
  ...
  componentWillUnmount(){
    const { dispatch } = this.props;
     dispatch({
      type:'appControl/clearTags'
    })
    dispatch({
      type:'appControl/clearPorts'
    })
    dispatch({
      type:'appControl/clearInnerEnvs'
    })
    dispatch({
      type:'appControl/clearStartProbe'
    })
    dispatch({
      type:'appControl/clearRunningProbe'
    })
    dispatch({
      type:'appControl/clearMembers'
    })
  }
  fetchBaseInfo = () => {
     const { dispatch } = this.props;
  ...
  handleEditHealth = (vals) => {
    ...
    if(appProbeUtil.isUsed(this.state.editStartHealth)){
        this.props.dispatch({
  ...
  handleEditRunHealth = (vals) => {
    if(appProbeUtil.isUsed(this.state.editRunHealth)){
        this.props.dispatch({
  ...
  render() {
    ...
    const { innerEnvs, runningProbe, startProbe, ports, baseInfo, appDetail, tags, teamControl } = this.props;
    ...
    return (
      ...
      <Form>
          {
            ...
            <FormItem style={{marginBottom: 0}} {...formItemLayout} label="镜像名称">
               {appDetail.service.image}
            </FormItem>
            :''
          }
          <FormItem style={{marginBottom: 0}} {...formItemLayout} label="创建时间">
             {baseInfo.create_time || ''}
          </FormItem>
          {
            tags ?
            <FormItem style={{marginBottom: 0}} {...formItemLayout}  label="应用特性">
              {
                 (tags.used_labels||[]).map((tag)=>{
                    return <Tag closable onClose={(e)=>{e.preventDefault(); this.handleRemoveTag(tag)}}>{tag.label_alias}</Tag>
                 })
              }
          ...
          
          {
            baseInfo.git_url ? 
            <Fragment>
            <FormItem style={{marginBottom: 0}} {...formItemLayout}  label="Git仓库">
      ...
      <Table 
        columns={[{
          ...
          render: (val, data) => {
             return (
              <Fragment>
                <a href="javascript:;" onClick={()=>{this.onDeleteVar(data)}}>删除</a>
      ...
      <Table 
        columns={[{
          ...
          render: (val, data, index) => {
            ...
            if(index === 0){
              if(appProbeUtil.isStartProbe(data)){
                 return (
                    <Fragment>
                      {appProbeUtil.isUsed(data) && <a href="javascript:;" onClick={()=>{this.showViewStartHealth(data)}}>查看</a>}
                      {<a href="javascript:;" onClick={()=>{this.setState({editStartHealth: data})}}>设置</a>}
                    </Fragment>
                   )
              }else{
                  return <a href="javascript:;" onClick={()=>{this.setState({editStartHealth: {data}})}}>设置</a>
              }
            }

            
            if(index ===1){
                if(appProbeUtil.isRunningProbe(data)){
                   return (
                      <Fragment>
                       {appProbeUtil.isUsed(data) && <a href="javascript:;" onClick={()=>{this.showViewRunningHealth(data)}}>查看</a>}
                        <a href="javascript:;" onClick={()=>{this.setState({editRunHealth: data})}}>设置</a>
                      </Fragment>
                     )
                }else{
                   return <a href="javascript:;" onClick={()=>{this.setState({editRunHealth: data})}}>设置</a>
                }
            }
     ...
     >
      <Table 
        columns={[
           {
            ...
            },
            {
              ...
              render(val, data) {
                return <div>
                    <a onClick={()=>{self.onEditAction(data)}} href="javascript:;">编辑权限</a>
                    <a onClick={()=>{self.onDelMember(data)}} href="javascript:;">移除应用权限</a>
                </div>
       ...
       <div  style={{marginTop:10, textAlign: 'right'}}>
        <Button  onClick={this.showAddMember}><Icon type="plus" /> 设置成员应用权限</Button>
       </div>
     </Card>

     {this.state.addTag && <AddTag tags={tags ? tags.unused_labels : []} onCancel={this.cancelAddTag} onOk={this.handleAddTag}  />}
     {this.state.showAddVar && <AddVarModal onCancel={this.handleCancelAddVar} onSubmit={this.handleSubmitAddVar}  />}
     ...
     {this.state.viewStartHealth && <ViewHealthCheck title="启动时检查查看" data={this.state.viewStartHealth} onCancel={()=>{this.setState({viewStartHealth: null})}} />}
     {this.state.editStartHealth && <EditHealthCheck ports={ports} onOk={this.handleEditHealth} title="设置启动时检查" data={this.state.editStartHealth} onCancel={this.onCancelEditStartProbe} />}
     {this.state.toEditAction && <EditActions onSubmit={this.handleEditAction} onCancel={this.hideEditAction} actions={teamControl.actions} value={this.state.toEditAction.identity}  />}
     {this.state.viewRunHealth && <ViewRunHealthCheck title="运行时检查查看" data={this.state.viewRunHealth} onCancel={()=>{this.setState({viewRunHealth: null})}} />}
     {this.state.editRunHealth && <EditRunHealthCheck ports={ports} onOk={this.handleEditRunHealth} title="设置运行时检查" data={this.state.editRunHealth} onCancel={this.onCancelEditRunProbe} />}
     {this.state.showAddMember && <SetMemberAppAction members={members} actions={teamControl.actions} onOk={this.handleAddMember} onCancel={this.hideAddMember} />}
     {this.state.toDeleteMember && <ConfirmModal onOk={this.handleDelMember} title="删除成员权限" desc="确定要删除此成员的应用权限吗？" onCancel={this.hideDelMember} />}
     </Fragment>
    );
  }
}
