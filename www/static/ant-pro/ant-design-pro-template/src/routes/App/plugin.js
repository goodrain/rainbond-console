...
import { Row, Col, Card, Form, Input, Button, Icon, Menu, Dropdown, List, Radio, Avatar, notification, Divider, Select} from 'antd';
...
class ConfigItems extends PureComponent {
   ...
   renderItem = (item, index) => {
      if(item.attr_type === 'string'){
          return <FormItem
            style={{width: '90%'}}
            label={<div>{item.attr_name} {item.attr_info ? <span>({item.attr_info})</span> : ''}</div>}
          > 
            <Input
      ...
      if(item.attr_type === 'radio'){
          ...
          >
            <Select
             ...
             disabled={!item.is_change} defaultValue={item.attr_value || item.attr_default_value || ''}>
                {
                  options.map((v) => {
      ...
      if(item.attr_type === 'checkbox'){
          ...
          >
            <Select
              disabled={!item.is_change}
              onChange={(val)=>{this.onChange(val.join(','), index)}}
...
class ConfigDownstreamPort extends PureComponent {
    render(){
       const data = this.props.data;
       return (
          <Card style={{marginBottom: 24}}  type="inner" title={<div>
              <span style={{marginRight: 24}}>下游应用: {data.dest_service_cname}</span>
              <span style={{marginRight: 24}}>端口号: {data.port}</span>
              <span style={{marginRight: 24}}>端口协议: {data.protocol}</span>
            </div>}>
...
class ConfigUnDefine extends PureComponent {
    ...
    render(){
       const data = this.props.data;
       return (
          <Card style={{marginBottom: 24}} type="inner" title="">
            <ConfigItems  onChange={this.handleOnChange}  data={data.config} /> 
          </Card>
       )
    }
...
export default class Index extends PureComponent {
  ...
  renderInstalled = () => {
      ...
      return <List
        ...
        renderItem={item => (
          <div>
          <List.Item
            actions={[this.isOpenedPlugin(item) ? <a onClick={()=>{this.closePlugin(item)}} href="javascript:;">隐藏配置</a> : <a onClick={()=>{this.openPlugin(item)}} href="javascript:;">查看配置</a>, , appPluginUtil.isStart(item) ? <a onClick={()=>{this.handleStopPlugin(item)}} href="javascript:;">停用</a> : <a onClick={()=>{this.handleStartPlugin(item)}} href="javascript:;">启用</a> , <a onClick={()=>{this.onDeletePlugin(item)}} href="javascript:;">卸载</a>]}
          >
            <List.Item.Meta
              avatar={<Avatar src={item.logo} shape="square" size="large" />}
              title={<div><Link to={`/myplugns/${item.plugin_id}`}>{item.plugin_alias}</Link><p style={{fontSize: 12, color:'#dcdcdc'}}><span style={{marginRight: 24}}>类别： {pluginUtil.getCategoryCN(item.category)}</span><span>版本： {item.build_version}</span></p></div>}
          ...
          </List.Item>
          {
            ...
            <div style={{textAlign: 'right', marginBottom: 80}}>  
                ...
                <Button style={{marginRight: 8}} onClick={()=>{this.handleUpdateConfig(item.plugin_id, this.state.openedPlugin[item.plugin_id])}} type="primary">更新配置</Button>
                <Button onClick={()=>{this.closePlugin(item)}} type="default">隐藏配置</Button>
            </div>
            </Fragment>
            : null
          }
  ...
  renderUnInstalled = () => {
       ...
       return <List
        ...
        renderItem={item => (
          <List.Item
            actions={[<a onClick={()=>{this.installPlugin(item)}} href="javascript:;">开通</a>]}
          >
            <List.Item.Meta
              avatar={<Avatar src={item.logo} shape="square" size="large" />}
              title={<div><Link to={`/myplugns/${item.plugin_id}`}>{item.plugin_alias}</Link><p style={{fontSize: 12, color:'#dcdcdc'}}><span style={{marginRight: 24}}>类别： {pluginUtil.getCategoryCN(item.category)}</span><span>版本： {item.build_version}</span></p></div>}
    ...
    return (
     <Card>
          <p style={{overflow: 'hidden'}}>
            <RadioGroup onChange={this.handleTypeChange} value={type} style={{marginRight: 16, float: 'left'}}>
              <RadioButton value="installed">已开通</RadioButton>
              <RadioButton value="uninstalled">未开通</RadioButton>
            </RadioGroup>
            <RadioGroup onChange={this.handleCategoryChange} defaultValue="" style={{marginRight: 16, float: 'right'}}>
          ...
          {this.state.showDeletePlugin && <ConfirmModal onOk={this.hanldeUnInstallPlugin} onCancel={this.cancelDeletePlugin} title="卸载插件" desc="确定要卸载此插件吗？" />}
     </Card>
    );
  }
}
