...
import { DatePicker, Row, Col, Card, Form, Button, Table, Icon, Menu, Dropdown, Tooltip} from 'antd';
...
class Empty extends PureComponent {
   render() {
      return (
        <div style={{height: '300px', lineHeight: '300px', textAlign:' center', fontSize: 26}}>暂无数据</div>
      )
   }
}
...
class MonitorNow extends PureComponent {
  ...
  componentWillUnmount(){
     ...
     this.destroySocket();
     this.props.dispatch({
      type: 'appControl/clearOnlineNumberRange'
     })
     this.props.dispatch({
     ...
     this.props.dispatch({
      type: 'appControl/clearOnlineNumber'
     })
  }
  getSocketUrl = () => {
  ...
  fetchRequestTime(){
    ...
    this.props.dispatch({
      ...
      payload: {
         team_name: globalUtil.getCurrTeamName(),
         app_alias: this.props.appAlias,
         serviceId: this.props.appDetail.service_id
      },
      complete: ()=> {
           if(this.mounted){
              setTimeout(()=>{
  ...
  render() {
    ...
    return (
     ...
     <div style={{textAlign: 'right', marginBottom: 25}}>
        <ButtonGroup>
            <Button onClick={()=>{this.changeType('now')}} type={ type === 'now' ? 'primary' : ''}>实时</Button>
            <Button onClick={()=>{this.changeType('history')}} type={ type === 'history' ? 'primary' : ''}>历史</Button>
        </ButtonGroup>
     </div>
     {
        type === 'now' ? 
