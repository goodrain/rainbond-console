...
class ShareEvent extends React.Component {
    ...
    getShareStatus = () => {
         ...
         this.props.dispatch({
           ...
           callback: (data) => {
              this.setState({status: data.bean.event_status}, () => {
    ...
    startShareEvent = () => {
        this.props.dispatch({
           ...
           callback: (data) => {
              this.setState({eventId: data.bean.event_id, status: data.bean.event_status}, () => {
                  this.getShareStatus();
              })
           }
        })
    ...
    render() {
      ...
      return (
        <div style={{marginBottom: 24}}>
          <h4>应用名称: {data.service_name} {this.renderStatus()}</h4>
          <div>
...
export default class shareCheck extends PureComponent {
     ...
     getShareEventInfo = () => {
        ...
        this.props.dispatch({
           ...
           callback: (data) => {
               this.setState({shareEventList: data.bean.event_list || [], status: !data.bean.is_compelte ? 'checking' : 'success'})
           }
        })
     }
     getParams = () => {
     ...
     handleCompleteShare = () => {
        ...
        this.props.dispatch({
           ...
           callback: (data) => {
              this.props.dispatch(routerRedux.replace("/groups/"+params.groupId))
           }
        })
     }
     handleGiveUp = () => {
     ...
     renderBody = () => {
        ...
        const extra = (
          <div>
              {
                (eventList || []).map((item) => {
                    return <ShareEvent onFail={this.handleFail}  onSuccess={this.handleSuccess} share_id={params.shareId} data={item} />
                })
              }
          </div>
        );
         ...
         if(status === 'success'){
           ...
           title="应用同步成功"
           desc = ""
           actions = [<Button  onClick={this.handleCompleteShare} type="primary">确认分享</Button>];
         }
         if(status === 'checking'){
           type = 'ing'
           title="应用同步中"
         ...
         if(status === 'failure'){
           ...
           actions = [<Button  onClick={this.handleReStart} type="primary">重新同步</Button>, <Button  onClick={this.showDelete} type="default">放弃分享</Button>];
         }
        return <Result
              type={type}
              title={title}
     ...
     render(){
        ...
        return <PageHeaderLayout>
          ...
          {this.state.showDelete && <ConfirmModal onOk={this.handleGiveUp} onCancel={this.hideShowDelete} title="放弃分享" desc="确定要放弃此次分享吗?" />}
        </PageHeaderLayout>
     }
}