...
import { Row, Col, Card, Form, Button, Icon, Menu, Dropdown, notification, Alert} from 'antd';
...
export default class Index extends PureComponent {
	...
	handleSubmit = (value) => {
        ...
        this.props.dispatch({
        	...
        	callback: (data)=>{
        		...
        		this.props.dispatch(routerRedux.push('/create/create-check/'+appAlias));
        	}
        })
	}
	...
	renderRegisterGitlabSuccess = () => {
        ...
        const extra = (
          <div>
            <p>用户名: {user.email} 或 {user.phone||'手机号'}</p>
            <p>密&nbsp;&nbsp;&nbsp;&nbsp;码: 同云帮登录密码</p>
          </div>
        );
        const actions = [<Button onClick={()=>{this.setState({registerGitlabSuccess: false})}} type="primary">点击继续创建应用</Button>];
        return <Result
              type="success"
              title="好雨Git账号注册成功"
		...
		return (
			...
			>
...

				{
					...
					<div>
						<Alert
					      message="请先注册您的好雨Git账号"
					      type="warning"
					      closable
					    />
					    <div style={{marginBottom: 32}}></div>
						<GitlabAccountForm data={{email: user.email}} onSubmit={this.handleAccountSubmit} />
					</div>
					: 
					<div>
