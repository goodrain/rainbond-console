...
import { Button, Icon, Card, Modal, Row, Col, Switch, Table, Radio, Tabs, Affix , Input} from 'antd';
...
@connect(({ user, appControl, teamControl, createApp }) => ({
    ...
    handleBuild = () => {
        ...
        buildApp({
          ...
          app_alias: app_alias
            ...
            if(data){
                this.props.dispatch(routerRedux.push('/app/'+app_alias+'/overview'));
            }
        })
     }
     handleDelete = () => {
     ...
     render(){
        ...
        return (<div>
          <h2 style={{textAlign: 'center'}}>高级设置</h2>
          <div style={{overflow:'hidden'}}>
            <AppCreateSetting appDetail={appDetail} />
            <div style={{background: '#fff',padding: '20px', textAlign: 'right', position: 'fixed', bottom: 0, left:0, right:0 , zIndex:2, borderTop: '1px solid #e8e8e8'}}>
                <Button style={{marginRight: 8}} onClick={this.handleBuild} type="primary">确认构建</Button> 
