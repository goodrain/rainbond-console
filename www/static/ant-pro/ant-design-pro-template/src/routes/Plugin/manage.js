...
import {Row, Col, Card, Button, Icon, List, Form, Select, Table, Notification, Dropdown, Menu } from 'antd';
...
export default class Index extends PureComponent {
     ...
     this.props.dispatch({
         ...
         callback: (data) => {
            ...
            if(!this.state.currVersion && data.list.length){
                 this.setState({currVersion: data.list[0].build_version}, () => {
  ...
  handleVersionChange = (val) => {
      val = val.key;
      if(val === this.state.currVersion) return;
      this.setState({currVersion: val}, () => {
           this.getPluginVersionInfo();
           this.getPluginVersionConfig();
      })
  }
  ...
  render() {
    ...
    if(!this.state.currInfo) return null;
    ...
    const extra = (
      <Row style={{float: 'right', width: 300}}>
        <Col xs={24} sm={12}>
      ...
      >
        ...
        >
          <div style={{maxWidth: 500, margin: '0 auto'}}>
            <CreatePluginForm allDisabled={!this.canEditInfoAndConfig()} isEdit={true} onSubmit={this.hanldeEditSubmit} data={this.state.currInfo} submitText="确认修改" />
          </div>
        </Card>
              ...
              columns={[{
                ...
                render: (v, data) => {
                     return (v||[]).map((item) => {
                         ...
                         <span>可否修改: {item.is_change? '可修改': '不可修改'}</span>
                ...
                render: (v, data) => {
                    ...
                    return (
                    <Fragment>
                      <a href="javascript:;"  onClick={()=>{this.showEditConfig(data)}} style={{marginRight: 8}}>修改</a>
                      <a href="javascript:;" onClick={()=>{this.handleDelConfig(data)}}>删除</a>
                    </Fragment>
                    )
                }
              }]}
        ...
        {this.state.showAddConfig && <AddOrEditConfig onCancel={this.hiddenAddConfig} onSubmit={this.handleAddConfig} />}
        {this.state.showEditConfig && <AddOrEditConfig data={this.state.showEditConfig} onCancel={this.hideEditConfig} onSubmit={this.handleEditConfig} />} 
        {this.state.showDeleteVersion && <ConfirmModal onOk={this.handleDeleteVersion} onCancel={this.cancelDeleteVersion} title="删除版本" desc="确定要删除当前版本吗？" subDesc="此操作不可恢复" />}
        {this.state.showBuildLog && this.state.currVersion && <BuildPluginVersion onCancel={this.hideBuildLog} event_id={this.state.event_id} plugin_id={this.getId()} build_version={this.state.currVersion} />}
      </PageHeaderLayout>
    );
  }
}
