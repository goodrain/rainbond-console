...
import { Row, Col, Card, List, Avatar, Button, Icon, Modal, Form, Input} from 'antd';
  ...
  handleAddMember = (values) => {
      ...
      this.props.dispatch({
        ...
        payload:{
          ...
          user_ids: values.user_ids.map((item)=>{return item.key}).join(','),
    ...
    const extraContent = (
      <div className={styles.extraContent}>
         <div className={styles.extraBtns}>
              {<Button disabled={!teamUtil.canDeleteTeam(team)} onClick={this.showDelTeam} type="dashed">删除团队</Button>}
         </div>
      </div>
    );
    ...
    return (
      ...
      >
        <Card
          ...
          title="已开通数据中心"
          bordered={false}
          extra={<a href="javascript:;" onClick={this.onOpenRegion}>开通数据中心</a>}
        ...
        >
          ...
          }
          {
            ...
            <p style={{textAlign: 'center',paddingTop: 20}}>暂无数据中心</p>
            :''
          }
        </Card>
        ...
        <Card
          ...
          bordered={false}
          title="团队成员"
          extra={teamUtil.canAddMember(team) ? <a href="javascript:;" onClick={this.showAddMember}>添加成员</a> : null}
        >
          <TeamMemberTable team={team} onMoveTeam={this.onMoveTeam} onDelete={this.onDelMember} onEditAction={this.onEditAction} list={members} />
        </Card>
        {this.state.showEditName && <MoveTeam teamAlias={team.team_alias} onSubmit={this.handleEditName} onCancel={this.hideEditName} />}
        {this.state.showDelTeam && <ConfirmModal onOk={this.handleDelTeam} title="删除团队" subDesc="此操作不可恢复" desc="确定要删除此团队吗？" onCancel={this.hideDelTeam} />}
        {this.state.toDeleteMember && <ConfirmModal onOk={this.handleDelMember} title="删除成员" subDesc="此操作不可恢复" desc="确定要删除此成员吗？" onCancel={this.hideDelMember} />}
        {this.state.toMoveTeam && <ConfirmModal onOk={this.handleMoveTeam} title="移交团队" subDesc="移交后您将失去所有权" desc={"确定要把团队移交给 "+ this.state.toMoveTeam.user_name + " 吗？"} onCancel={this.hideMoveTeam} />}
        {this.state.showAddMember && <AddMember actions={teamControl.actions} onOk={this.handleAddMember} onCancel={this.hideAddMember} />}
