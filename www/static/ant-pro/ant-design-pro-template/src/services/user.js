...
/*登录*/
export async function login(body={nick_name, password}) {
...
export async function logout() {
  return request(config.baseUrl + '/console/users/logout', {
  	 method: 'get'
  });
}

/*注册*/
export async function register(body={user_name, email, password, password_repeat, captcha_code}) {
...
/* 发送找回密码邮件 */
export async function send_backpassword_email(body={email}) {
...
/*重置密码*/
export async function reset_password(body={password, password_repeat}){
...
/*修改密码*/
export async function editPassword(body={password, new_password, new_password_repeat}){
...
export async function getDetail(handleError){
   return request(config.baseUrl + '/console/users/details', {
...
/*
	模糊查询用户
*/
export async function search(body = {key}){
...
export async function gitlabRegister(body={email, password}){
...
/*
  创建github项目
*/
export async function createGitlabProject(body={team_name, project_name}){
