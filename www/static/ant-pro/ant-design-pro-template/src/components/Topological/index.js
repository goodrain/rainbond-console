...
class Index extends React.Component {
  ...
  componentWillMount(){
      ...
      try{
          ...
          window.handleClickService = function(nodeDetails){
            self.props.dispatch(routerRedux.push("/app/"+nodeDetails.service_alias+"/overview"))
          }

          //拓扑图点击依赖服务事件
          window.handleClickRelation = function(relation){
            self.props.dispatch(routerRedux.push("/app/"+relation.service_alias+"/overview"))
          }
  ...
  render() {
    return (
        (
          <iframe src='/static/www/weavescope/index.html' style={{width: '100%', height: '500px'}} frameborder="no" border="0" marginwidth="0" marginheight="0" ></iframe>
        )
    );
  }
}
