											...
											<Col span="12">
												...
												<div id={order.name + '-memory'} style={{width:'100%',height:'160px'}}>
													<Echars style={{height: 150, width: 150}} option={{
														color:[
															(order.memory.used ===0 && order.memory.limit === 0) ? blackColor : (order.memory.used > order.memory.limit ? warningColor : normalColor)
															, blackColor],
														series: [
															{
																name:'访问来源',
												...
												<p>
													...
													<span className={styles.usedAndLimit}>
														...
														<br />
														{sourceUtil.unit((order.memory.limit < order.memory.used ? order.memory.used - order.memory.limit : 0), 'MB')}
													</span>
												</p>
											</Col>
											<Col span="12">
												...
												<Echars style={{height: 150, width: 150}} option={{
														color:[
															(order.disk.used ===0 && order.disk.limit === 0) ? blackColor : (order.disk.used > order.disk.limit ? warningColor : normalColor)
															, blackColor],
														series: [
															{
																name:'访问来源',
												...
												<p>
													...
													<span className={styles.usedAndLimit}>
														<span className={styles.tit}>按需计费 <Tooltip title="举例： 当前使用10G， 当前包月5G，则10G-5G=5G为按需计费的量"><Icon type="info" /></Tooltip></span>
														<br />
														{sourceUtil.unit((order.disk.limit < order.disk.used  ? order.disk.used - order.disk.limit : 0), 'GB')}
													</span>
												</p>
											</Col>
										</Row>
