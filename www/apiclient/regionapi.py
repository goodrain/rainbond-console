...
class RegionInvokeApi(RegionApiBaseHttpClient):
    ...
    def get_cluster_gpu_overview(self, region):
        """获取集群 GPU 总览"""
        region_info = self.get_region_info(region)
        if not region_info:
            raise ServiceHandleException("region not found")
        url = region_info.url + "/v2/cluster/gpu-overview"
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def get_node_gpu_detail(self, region, node_name):
        """获取节点 GPU 详情"""
        region_info = self.get_region_info(region)
        if not region_info:
            raise ServiceHandleException("region not found")
        url = region_info.url + "/v2/cluster/nodes/{0}/gpu".format(node_name)
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def get_available_gpu_models(self, region):
        """获取可用 GPU 型号列表"""
        region_info = self.get_region_info(region)
        if not region_info:
            raise ServiceHandleException("region not found")
        url = region_info.url + "/v2/gpu-models"
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def detect_hami_status(self, region):
        """检测 HAMi 安装状态"""
        region_info = self.get_region_info(region)
        if not region_info:
            raise ServiceHandleException("region not found")
        url = region_info.url + "/v2/cluster/hami-status"
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def get_tenant_gpu_quota(self, region, tenant_name):
        """获取团队 GPU 配额"""
        url, token = self.__get_region_access_info(tenant_name, region)
        tenant_region = self.__get_tenant_region_info(tenant_name, region)
        url = url + "/v2/tenants/" + tenant_region.region_tenant_name + "/gpu-quota"
        self._set_headers(token)
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def set_tenant_gpu_quota(self, region, tenant_name, data):
        """设置团队 GPU 配额"""
        url, token = self.__get_region_access_info(tenant_name, region)
        tenant_region = self.__get_tenant_region_info(tenant_name, region)
        url = url + "/v2/tenants/" + tenant_region.region_tenant_name + "/gpu-quota"
    ...
    def get_tenant_gpu_usage(self, region, tenant_name):
        """获取团队 GPU 使用情况"""
        url, token = self.__get_region_access_info(tenant_name, region)
        tenant_region = self.__get_tenant_region_info(tenant_name, region)
        url = url + "/v2/tenants/" + tenant_region.region_tenant_name + "/gpu-usage"
        self._set_headers(token)
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def get_service_gpu_config(self, region, tenant_name, service_alias):
        """获取组件 GPU 配置"""
        url, token = self.__get_region_access_info(tenant_name, region)
        tenant_region = self.__get_tenant_region_info(tenant_name, region)
        url = url + "/v2/tenants/" + tenant_region.region_tenant_name + "/services/" + service_alias + "/gpu-config"
        self._set_headers(token)
        res, body = self._get(url, self.default_headers, region=region)
        return res, body

    def set_service_gpu_config(self, region, tenant_name, service_alias, data):
        """设置组件 GPU 配置"""
        url, token = self.__get_region_access_info(tenant_name, region)
        tenant_region = self.__get_tenant_region_info(tenant_name, region)
        url = url + "/v2/tenants/" + tenant_region.region_tenant_name + "/services/" + service_alias + "/gpu-config"
    ...
    def delete_service_gpu_config(self, region, tenant_name, service_alias):
        """删除组件 GPU 配置"""
        url, token = self.__get_region_access_info(tenant_name, region)
        tenant_region = self.__get_tenant_region_info(tenant_name, region)
        url = url + "/v2/tenants/" + tenant_region.region_tenant_name + "/services/" + service_alias + "/gpu-config"
    ...
    def restore_cluster_from_backup(self, region_name, old_service_id, new_service_id, backup_name):
        ...
        if not region_info:
            raise ServiceHandleException("region not found")
        url = region_info.url
        url += f"/v2/cluster/kubeblocks/clusters/{old_service_id}/restores"
        self._set_headers(region_info.token)
