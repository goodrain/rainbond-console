...
class QingCloudApi(BaseHttpClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        api_info = settings.QING_CLOUD_API
    ...
    def make_proxy_http(self, region_service_info):
        ...
        proxy = httplib2.ProxyInfo(proxy_type, proxy_info[
                                   'host'], proxy_info['port'])
    ...
    def _request(self, *args, **kwargs):
        ...
        response, content = super(RegionServiceApi, self)._request(
            client=client, *args, **kwargs)
        return response, content

    def create_service(self, region, tenant, body):
        url = self.region_map[region]['url'] + \
            "/v1/tenants/" + tenant + "/services"
    ...
    def update_service(self, region, service_id, data):
        ...
        res, body = self._put(url, self.default_headers,
                              json.dumps(data), region=region)
        return res, body

    def build_service(self, region, service_id, body):
        url = self.region_map[region]['url'] + \
            "/v1/services/lifecycle/" + service_id + "/build/"
        res, body = self._post(url, self.default_headers, body, region=region)
        return body

    def check_service_status(self, region, service_id):
        url = self.region_map[region]['url'] + \
            "/v1/services/lifecycle/" + service_id + "/status/"
        res, body = self._post(url, self.default_headers, region=region)
        return body

    def restart(self, region, service_id, body):
        url = self.region_map[region]['url'] + \
            "/v1/services/lifecycle/" + service_id + "/restart/"
        res, body = self._post(url, self.default_headers, body, region=region)
        return body

    def stop(self, region, service_id):
        url = self.region_map[region]['url'] + \
            "/v1/services/lifecycle/" + service_id + "/stop/"
        res, body = self._post(url, self.default_headers, region=region)
        return body

    def delete(self, region, service_id):
        url = self.region_map[region]['url'] + \
            "/v1/services/lifecycle/" + service_id + "/delete/"
    ...
    def changeMemory(self, region, service_id, body):
        url = self.region_map[region]['url'] + \
            "/v1/services/" + service_id + "/language"
        res, body = self._post(url, self.default_headers, body, region=region)
        return body
    ...
    def unpause(self, region, tenant_id):
        url = self.region_map[region]['url'] + \
            "/v1/tenants/" + tenant_id + "/unpause"
        res, body = self._post(url, self.default_headers, region=region)
        return body

    def writeToRegionBeanstalk(self, region, service_id, body):
        url = self.region_map[region][
            'url'] + "/v1/services/lifecycle/" + service_id + "/beanstalk/"
        res, body = self._post(url, self.default_headers, body, region=region)
        return body

    def createServiceDependency(self, region, service_id, body):
        url = self.region_map[region][
            'url'] + "/v1/services/lifecycle/" + service_id + "/dependency/"
        res, body = self._post(url, self.default_headers, body, region=region)
        return body
    ...
    def createServiceEnv(self, region, service_id, body):
        url = self.region_map[region]['url'] + \
            "/v1/services/lifecycle/" + service_id + "/env-var/"
    ...
    def opentsdbQuery(self, region, start, queries):
        ...
        try:
            ...
            logger.info(
                'tsdb_query', "response: {0} ====== {1}".format(res, body))
            return None
