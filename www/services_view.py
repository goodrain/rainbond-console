...
class TenantService(LeftSideBarMixin, AuthedView):
	...
	def get(self, request, *args, **kwargs):
		...
		context = self.get_context()
		try:
			num = TenantServiceInfo.objects.filter(tenant_id=self.tenant.tenant_id,
												   service_region=self.response_region).count()
			# if num < 1:
			#     return self.redirect_to('/apps/{0}/app-create/'.format(self.tenant.tenant_name))
			tenantServiceList = context["tenantServiceList"]
			context["totalAppStatus"] = "active"
	...
	def get_manage_app(self, http_port_str):
		...
		if self.service.service_key == 'mysql':
			...
			if has_managers:
				...
				manager = has_managers[0]
				service_manager[
					'url'] = 'http://{0}.{1}.{2}{3}:{4}'.format(manager.service_alias, self.tenant.tenant_name,
																self.service.service_region, settings.WILD_DOMAIN,
																http_port_str)
			else:
				service_manager['url'] = '/apps/{0}/service-deploy/?service_key=phpmyadmin'.format(
		...
		try:
			...
			if fr == "deployed":
				...
				context["envs"] = TenantServiceEnvVar.objects.filter(service_id=self.service.service_id,
																	 scope="inner").exclude(container_port=-1)
	...
	def get(self, request, *args, **kwargs):
		...
		try:
			...
			if docker_c_id != "" and docker_h_id != "" and docker_s_id != "" and docker_s_id == self.service.service_id:
				...
				context["wss"] = settings.DOCKER_WSS_URL.get("type", "ws") + "://" + docker_h_id + \
								 settings.DOCKER_WSS_URL[self.service.service_region] + "/ws"
