...
class AppDeploy(AuthedView):
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			envs = {}
			buildEnvs = TenantServiceEnvVar.objects.filter(service_id=service_id,
	...
	def get(self, request, *args, **kwargs):
		...
		try:
			service_list = TenantServiceInfo.objects.filter(
				tenant_id=self.tenant.tenant_id, service_region=self.cookie_region).values('ID', 'service_id',
																						   'deploy_version')
...
class ServiceDomainManager(AuthedView):
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			# add container port
			container_port = request.POST["multi_port_bind"]
			if match:
				result["status"] = "failure"
				return JsonResponse(result)
			...
			if action == "start":
				...
				if num == 0:
					...
					# add domain port
					domain["container_port"] = container_port
				...
				# sysc region add container port
...
class ServiceMnt(AuthedView):
	...
	def post(self, request, *args, **kwargs):
		result = {}
		action = request.POST["action"]  # 获取POST请求参数
		dep_service_alias = request.POST["dep_service_alias"]
		try:
			tenant_id = self.tenant.tenant_id
			service_id = self.service.service_id
