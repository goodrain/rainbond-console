...
class InviteTenantUser(AuthedView):
    ...
    def invite_content(self, email, tenant_name, identity):
        ...
        content = "尊敬的用户您好，"
        content = content + "<br/>"
        content = content + "非常感谢您申请试用 好雨云平台！ 请点击下面的链接完成注册:"
        content = content + "<br/>"
        content = content + "注册链接: " + link_url
        content = content + "<br/>"
        content = content + "我们的服务在一定的资源范围内永久免费！内测阶段也可以申请增加免费资源，增加的资源在产品正式版上线后也不会另收费用哦！另外参与内测并提交问题报告的用户，正式上线后还会有更多的福利。"
        content = content + "<br/>"
        content = content + "我们的文档及博客正在建设中，以后会陆续发布一系列好雨云平台的使用教程和技巧，欢迎关注！"
        content = content + "<br/>"
        content = content + "您在使用过程中遇到的任何问题，或者对平台有任何建议，都可以通过以下途径提交反馈。对于提出高质量的反馈的用户，还有精美礼品等待您！"
        content = content + "<br/>"
        content = content + "Email： ares@goodrain.com"
        content = content + "<br/>"
        content = content + "微信公众号：goodrain-cloud "
        content = content + "<br/>"
        content = content + "联系电话：13621236261"
        content = content + "<br/>"
        content = content + "再次感谢您关注我们的产品！"
        content = content + "<br/>"
        content = content + "好雨科技 (Goodrain Inc.) CEO 刘凡"
        return content
