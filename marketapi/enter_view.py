...
class EnterBindAPIView(BaseMarketAPIView):
    def post(self, request):
        ...
        if not api.auth_sso_user_token():
            ...
            return self.error_response(
                code=status.HTTP_403_FORBIDDEN, msg='illegal user token!')
        ...
        success = market_api.active_market_enterprise(
            sso_user, enterprise_id, market_client_id, market_client_token)
        if success:
            return self.success_response(data={'echostr': echostr})
        else:
            return self.error_response(status.HTTP_500_INTERNAL_SERVER_ERROR,
                                       'enterprise bind failed',
                                       '服务器错误,企业认证绑定失败')
    ...
    def post(self, request):
        ...
        if not region_name or not access_url or not access_token:
            return self.error_response(status.HTTP_400_BAD_REQUEST,
                                       'missing parameters', '参数不合法')

        if request.user.enterprise_id != enterprise_id:
            logger.debug('login_user.eid:{}'.format(
                request.user.enterprise_id))
            logger.debug('param.eid:{}'.format(enterprise_id))
            return self.error_response(status.HTTP_403_FORBIDDEN,
                                       'no access right for this api', '未授权')

        enter = market_api.get_enterprise_by_id(enterprise_id)
        if not enter:
            return self.error_response(status.HTTP_404_NOT_FOUND,
                                       'enterprise not fond!', '企业信息不存在!')

        market_api.save_region_access_token(enterprise_id, region_name,
                                            access_url, access_token, key, crt)
...
class RegionEnterResourceAPIView(EnterpriseMarketAPIView):
    def get(self, request):
        ...
        if not tenant:
            logger.error(
                LOGGER_TAG,
                'user [{0}] do not have default tenant!'.format(user.user_id))
            return self.error_response(
                code=status.HTTP_404_NOT_FOUND,
                msg='tenant does not exist!',
                msg_show='租户不存在')
        total_memory, total_disk, total_net = market_api.get_tenant_region_resource_usage(
            tenant, region)
        limit_memory, limit_disk, limit_net = market_api.get_tenant_region_resource_limit(
            tenant, region)
        data = {
    ...
    def put(self, request):
        ...
        if tenant.enterprise_id != enter.enterprise_id:
            logger.error(
                LOGGER_TAG,
                'user default tenant: {} not belong to enterprise: {}!'.format(
                    tenant.tenant_name, enter.enterprise_name))
            return self.error_response(
                code=status.HTTP_404_NOT_FOUND,
...
class EnterGroupServiceListAPIView(EnterpriseMarketAPIView):
    def get(self, request):
        ...
        if group_ids:
            logger.debug(
                LOGGER_TAG,
                'Receive request from market, group ids:{0}'.format(group_ids))
            group_list = market_api.get_group_services_by_pk_list(group_ids)
        elif region_name:
            logger.debug(
                LOGGER_TAG,
                'Receive request from market, region_name:{0}'.format(
                    region_name))
            ...
            if not tenant:
                logger.error(
                    LOGGER_TAG,
                    'user [{0}] do not have default tenant!'.format(
                        user.user_id))
                return self.error_response(
                    code=status.HTTP_404_NOT_FOUND,
                    msg='tenant does not exist!',
            ...
            group_list = market_api.list_group_service_by_region(
                tenant, region_name)
        else:
            group_list = list()
            ...
            if hasattr(group, 'service_list'):
                for s in group.service_list:
                    service_list.append({
                        ...
                        s.status,
                        'access_url':
                        s.access_url if s.status == 'running' else '',
                    })
    ...
    def post(self, request):
        ...
        message = "安装失败"
        try:
            success, message, group = market_api.install_service_group(
                request.user, tenant_name, group_key, group_version,
                region_name, template_version)
        ...
        if success and group:
            ...
            return self.error_response(
                code=status.HTTP_500_INTERNAL_SERVER_ERROR, msg_show=message)
...
class EnterGroupServiceAPIView(EnterpriseMarketAPIView):
    ...
    def put(self, request, group_id):
        ...
        if action == 'restart':
            success, message = market_api.restart_tenant_service_group(
                request.user, group_id)
        elif action == 'stop':
            success, message = market_api.stop_tenant_service_group(
                request.user, group_id)
        elif action == 'build':
            success, message = market_api.build_tenant_service_group(
                request.user, group_id)
    ...
    def get(self, request):
        ...
        for tenant in tenants:
            data.append({
                ...
                tenant.pay_type,
                'is_default':
                True if tenant.creater == user.user_id else False,
                'tenant_regions': [{
