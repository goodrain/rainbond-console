...
class TeamNoRegionAPIView(BaseOpenAPIView):
    ...
    def initial(self, request, *args, **kwargs):
        ...
        if team_id:
            ...
            self.team = team_services.get_enterprise_tenant_by_tenant_name(
                enterprise_id=self.enterprise.enterprise_id, tenant_name=team_id)
            if not self.team:
                self.team = team_services.get_team_by_team_id_and_eid(
                    team_id=team_id, enterprise_id=self.enterprise.enterprise_id)
            if not self.team:
