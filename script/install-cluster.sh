...
parse_args() {
    ...
    while [ $# -gt 0 ]; do
        ...
        "--etcd")
            ...
            RKE2_DISABLE_ROLE_ETCD=false
            if [ -z "$NODE_ROLES" ]; then
                NODE_ROLES="etcd"
            else
                NODE_ROLES="${NODE_ROLES},etcd"
            fi
            shift 1
                ;;
        "--worker")
            info "Role requested: worker"
            if [ "${RKE2_DISABLE_ROLE_CONTROLPLANE}" != "false" ] && [ "${RKE2_DISABLE_ROLE_ETCD}" != "false" ]; then
              INSTALL_RKE2_TYPE=agent
            fi
            if [ -z "$NODE_ROLES" ]; then
                NODE_ROLES="worker"
            else
                NODE_ROLES="${NODE_ROLES},worker"
            fi
            shift 1
            ;;
        "--external-ip")
...
register_node() {
    ...
    while true; do
        ...
        # server_ip is empty. Wait 5 seconds and try again
        sleep 5
    done
}
